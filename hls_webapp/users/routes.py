...
def account():
    ...
    if form.validate_on_submit():
        ...
        audio_file = SoundFileIn(file_name=audio_file, user_id=current_user.id)
        ...
        sim_data = DecibelLoss(audiogram_name=audiogram_name, hll125=left['hlleft125'], hlr125=right['hlright125'], hll250=left['hlleft250'], hlr250=right['hlright250'], hll500=left['hlleft500'], hlr500=right['hlright500'], hll1000=left['hlleft1000'], hlr1000=right['hlright1000'],
                               hll2000=left['hlleft2000'], hlr2000=right['hlright2000'], hll4000=left['hlleft4000'], hlr4000=right['hlright4000'], hll8000=left['hlleft8000'], hlr8000=right['hlright8000'], compress=form.compression_loss.data, user_id=current_user.id)
...
def user_simulation(username):
    ...
    if form.validate_on_submit():
        ...
        compute(in_file, out_file, left_p_loss, right_p_loss)

        session["sound_out"] = "/" + path_out + "sim" + file_name
        # save the new file to db
        audio_file = SoundFileOut(file_name=out_file, user_id=current_user.id)
        db.session.add(audio_file)
        db.session.commit()
